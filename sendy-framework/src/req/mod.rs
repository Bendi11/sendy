...
pub trait StatefulToBytes {
    ...
    fn stateful_size_hint(&self, _: SerializationState<'_>) -> Option<usize> {
        None
    }
...
impl<T: ToBytes> StatefulToBytes for T {
    fn stateful_write<B: BufMut>(&self, _: SerializationState<'_>, buf: B) {
        <Self as ToBytes>::write(self, buf)
    }
    fn stateful_size_hint(&self, _: SerializationState<'_>) -> Option<usize> {
        <Self as ToBytes>::size_hint(self)
    }
}
impl<T: FromBytes> StatefulFromBytes for T {
    fn stateful_parse(
        _: SerializationState<'_>,
        buf: &mut untrusted::Reader<'_>,
    ) -> Result<Self, FromBytesError> {
    ...
    fn stateful_write<W: BufMut>(&self, state: SerializationState<'_>, mut buf: W) {
        ...
        let encrypted =
            match state
                ...
                .remote_keys()
                .enc
                .encrypt(&mut OsRng, Pkcs1v15Encrypt, &bytes)
            {
                Ok(b) => b,
                Err(e) => {
...
impl<T: StatefulFromBytes> StatefulFromBytes for Signed<T> {
    fn stateful_parse(
        ...
        if let Err(e) = state
            ...
            .remote_keys()
            .auth
            .verify(read.as_slice_less_safe(), &sig)
        {
            return Err(FromBytesError::Parsing(format!(
                "Failed to verify signature from {}: {}",
                state.peer.remote().ip(),
                e
            )));
        }
