...
mod secserv {
    ...
    use sendy_framework::{ToBytes, FromBytes, rsa::RsaPrivateKey};
    use super::*;
    ...
    impl SecretStore for SecretServiceStore {
        async fn store(&self, keys: &PrivateKeychain) {
            ...
            if let Err(e) = collection.create_item(
                ...
                HashMap::from([
                    (Self::SENDY_ATTR, Self::AUTHENTICATION_KEY_LABEL)
                ]),
                &auth_key,
                true,
            ...
            if let Err(e) = collection.create_item(
                ...
                HashMap::from([
                    (Self::SENDY_ATTR, Self::ENCRYPTION_KEY_LABEL)
                ]),
                &enc_key,
                true,
                Self::KEYS_CONTENT_TYPE,
        ...
        async fn read(&self) -> Option<PrivateKeychain> {
            ...
            match collection.search_items(HashMap::from([(Self::SENDY_ATTR, Self::ENCRYPTION_KEY_LABEL)])).await {
                Ok(enc) => match collection.search_items(HashMap::from([(Self::SENDY_ATTR, Self::AUTHENTICATION_KEY_LABEL)])).await {
                    Ok(auth) => {
                        let (auth, enc) = (auth.first()?, enc.first()?);

                        match auth.get_secret().and_then(|auth| enc.get_secret().map_ok(|enc| (auth, enc))).await {
                            Ok((auth, enc)) => match RsaPrivateKey::read_from_slice(&auth)
                                .and_then(|auth| RsaPrivateKey::read_from_slice(&enc).map(|enc| (auth, enc))) {
                                Ok((auth, enc)) => Some(PrivateKeychain::new(auth, enc)),
                                Err(e) => {
                                    log::error!("Failed to decode RSA keys from secrets: {}", e);
                                    None
                                }
                            },
                            Err(e) => {
